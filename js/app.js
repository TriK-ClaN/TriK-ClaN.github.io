...
function getCurrentTime() {
  const date = new Date("8/22/2022 00:00:00"); // Updated start date
  ...
  let cDate = new Date();
  let cMonth = cDate.getMonth() + 1; // JavaScript months are 0-indexed
  let cYear = cDate.getFullYear();
  let timeC = dateRange(2022, 8, cYear, cMonth); // Adjusted to include precise date
  let tempDay = cDate.getDate();

  // Calculate remaining days of the first month
  let daysInStartMonth = getNdays(8, 2022);
  let daysFromStart = daysInStartMonth - 22; // Days remaining in August after the 22nd

  // year count
  let countYear = Math.floor((timeC.length - (tempDay < 22 ? 1 : 0)) / 12); // Adjusted for partial first month
  ...
  // month count
  let countMonth = ((timeC.length - (tempDay < 22 ? 1 : 0)) % 12) - 1; // Account for partial start month
  countMonth = countMonth < 10 ? "0" + countMonth : countMonth;

  dm.innerText = countMonth;

  // Adjust tempDay for weeks and days calculation
  tempDay = tempDay + (tempDay < 22 ? daysFromStart : -22);
...
newDeg = null;
  setTimeout(function(){
    ...
    infoSpin.innerText=`Congratulations, my love! You will receive: ${wItem}!`
  },5000)
}
